...
from somsphere.utils import compute_distance, get_best_cell, get_sigma, get_alpha, is_power_2, timeit
...
class SOMap(object):
    ...
    def create_map(self, X, Y, importance=None, inputs_weights=None, random_order=True, eval_map=False):
        ...
        if self.som_type == SomType.ONLINE:
            self.__create_map_online(n_row=n_row, random_order=random_order, eval_map=eval_map)
    ...
    @timeit
    def __create_map_online(self, n_row, random_order=True, eval_map=False):
    ...
    def evaluate_map(self, input_x=None, input_y=None):
        ...
        for i in range(len(in_x)):
            inputs = in_x[i]
            best, activation = get_best_cell(inputs=inputs, importance=self.importance, weights=self.weights,
                                             n_pix=self.n_pix)
    ...
    def predict(self, line, best=True):
        ...
        """
        bests, _ = get_best_cell(inputs=line, importance=self.importance, weights=self.weights, n_pix=self.n_pix,
                                 return_vals=10)
        if best:
            return bests[0]
        for ib in range(10):
